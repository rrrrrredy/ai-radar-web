...
- Added public homepage, radar, clusters, entities, reports, admin, and auth callback routes.
...
- Emit ignored local outputs under `data/understanding/latest/` and `data/understanding/runs/`.

## Phase 6: Retrieval Foundation - Done

- Added retrieval over validated radar-item evidence.
- Kept source-cited evidence output with time windows and uncertainty for server-side report and quality workflows.
- Removed public assistant generation routes and APIs so the product stays centered on radar, entities, reports, and admin review.
...
- Added mutation gates requiring both `--write` and `ENABLE_SUPABASE_WRITES=true`.
...
- Moved public coverage panels to public snapshot/public view/feed-derived summaries so homepage, radar, and reports do not read service-role operational tables.

Deferred to later phases:

- Production Supabase mutations and scheduled jobs.
...
- Kept Supabase mutations, scheduled persistence, source-health history persistence, live DeepSeek, X API use, and WeChat auto-crawling disabled.
...
- Add review queues for `needs_review` items, source changes, and report candidates.
