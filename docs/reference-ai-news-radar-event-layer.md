...
The final RC has 254 public radar items, 198 current-run/public-display event clusters, 201 event relationships, 8 curated events, one same-family two-source event, and one cross-family event. The cross-family event comes from Anthropic and MIT Technology Review coverage and was enabled by a narrow tested concept alias, not a lower global similarity threshold. Product copy calls this cross-family coverage rather than independent confirmation because source independence is not established.
