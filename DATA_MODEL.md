...
Statuses: `draft`, `needs_review`, `approved`, `deferred`, `rejected`, `published`. Phase 9.4b can create, approve, defer, and reject candidates. Milestone C can turn approved daily, weekly, or topic candidates into `reports` rows with `reviewed` or `published` status and marks the candidate `published` only when the report record is published.
