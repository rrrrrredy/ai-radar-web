-- Release candidate: expose quality-gated evidence drafts as public-safe report candidates.
-- The projection remains read-only and keeps the existing field allowlist.
...
comment on view public.public_report_candidates is
  'Public-safe read-only display view for daily/weekly report candidates, including needs-review evidence drafts. Exposes candidate display fields and allowlisted report quality-gate metadata; excludes workflow actors, review notes, raw model metadata, operational metadata, secrets, and write access.';
